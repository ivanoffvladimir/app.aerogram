 ...
 *
 * Три числа, а не одно «готово»: пачку печатают целиком, и кладовщик должен
 * знать, скольких этикеток в ней не будет и стоит ли их подождать.
 */
export interface BatchLabels {
  ready: number
...
/**
 * Ответ на подключение второго фактора (`POST /v1/auth/mfa/setup`).
 *
 * Единственное место во всём API, где секрет виден, и только один раз:
 * повторно сервер его не отдаёт. Экран обязан сказать об этом до того,
 * как пользователь закроет страницу.
 */
