...
class LoginRequest(BaseModel):
    """Вход. Длина полей здесь НЕ проверяется, и это осознанно.

    Отказ во входе обязан выглядеть одинаково при любом неверном вводе.
    Ограничение снизу нарушало это: короткий пароль отбивался схемой
    до аутентификации и давал ``422`` с полем ``password``, а неверный
    длинный — ``401`` без поля. Разница выдавала нижнюю границу политики,
    то есть сужала перебор, — и заодно ломала то самое единообразие,
    ради которого вход не показывает причину отказа.

    Политика пароля живёт там, где ей место: при создании пользователя
    (``CreateUserRequest``, не меньше двенадцати). Цифра на входе была
    не политикой, а её устаревшей копией — восемь против двенадцати.

    Верхняя граница остаётся. Она не подсказка: узнать, что пароль
    не длиннее ста двадцати восьми символов, перебору не помогает,
    а неограниченный ввод — настоящая работа для Argon2 на каждой
    попытке.
    """

    email: EmailStr
    password: str = Field(max_length=128)
    #: Код TOTP. Обязателен для роли owner (12.5 ТЗ). Длина не проверяется
    #: по той же причине: код неверной длины — это неверный код, и отвечать
    #: на него нужно так же, как на неверный.
