...
CABINET_ONLY: frozenset[tuple[str, str]] = frozenset(
    {
        ...
        # Чтение снимка решения — работа кабинета: карточка отправления
        # и разбор спора. Машинному клиенту он не нужен: идентификатор
        # решения приходит ему в ответе на создание, а решение автовыбора —
        # вместе с рекомендацией. Открыть путь ключу можно только новой
        # областью ``decisions:read``, а состав областей — это права,
        # то есть решение человека (CLAUDE.md §7, пункт 6). Отдать его
        # под ``decisions:write`` было бы хуже: чтение под правом записи
        # читается как недосмотр и однажды им станет.
        ("GET", "/v1/decisions/{decision_id}"),
        # Правила маршрутизации — корпоративная политика. Чтение требует
        # логиста, правка владельца; у ключа роль машинного клиента, и область
        # здесь была бы обещанием, которое всё равно упирается в 403.
        # Отдельно важно, что политику не должен менять тот, кого она
        # ограничивает: интеграция считает по правилам, а не правит их.
