...
class ScoreBasis(StrEnum):
    """На чём стоит скор: на опыте платформы, своём или на обоих.

    Отдельно от ``ScoreConfidence`` намеренно. Доверие отвечает «насколько
    много данных», основание — «чьи это данные», и это разные вопросы:
    оценка платформы на тысяче чужих отправлений и оценка на своих тридцати
    могут иметь одинаковое доверие, но клиент обязан различать их, потому
    что договоры и направления у него свои.
    """

    #: Своих завершённых отправлений нет вовсе — показана оценка платформы.
    PLATFORM = "platform"
    #: Свои наблюдения есть и притянуты к платформенной базе.
    MIXED = "mixed"
    #: Платформенной базы нет, число посчитано только по своим наблюдениям.
    OWN = "own"
    #: Ни своих данных, ни платформенной базы: скора нет.
    NONE = "none"
