        ...
        for row in rows:
            ...
            try:
                recommendation = await self._recommendations.recommend(
                    ...
                    tenant_id=tenant_id,
                    # Автовыбор в прогоне НЕ участвует, и это решение, а не
                    # недосмотр (ADR-0029, раздел о массовых прогонах).
                    #
                    # Во-первых, стратегию прогона оператор выбрал для этих
                    # строк сам: правило владельца, срабатывающее поверх неё,
                    # — вторая автоматика с неопределённым старшинством.
                    # Во-вторых, ключ решения автовыбора выведен из РАСЧЁТА,
                    # а две одинаковые строки списка делят один расчёт
                    # (FR-1.6, переиспользование выдачи). Одно решение
                    # на две строки означало бы одно отправление на две:
                    # «одно решение — одно отправление» держит домен,
                    # и вторая строка просто не оформилась бы.
                    #
                    # Что здесь правильнее по существу — вопрос продуктовый,
                    # и он вынесен человеку в docs/status.md.
                    auto_select=False,
                )
