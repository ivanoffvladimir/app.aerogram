...
* политика повторов другая: у ДаData 403 повтором не лечится ни в одном
  из своих смыслов, а 429 повтором только продлевается.
...
class DadataClient:
    ...
    def _error_for_status(self, response: httpx.Response) -> DirectoryError | None:
        ...
        if status == 403:
            # 403 у ДаData значит ТРИ разные вещи, и различить их нечем:
            # по её же документации это «несуществующий ключ», «не подтверждена
            # почта» ИЛИ «исчерпан дневной лимит», и на все три приходит один
            # ответ — `Feature 'SUGGESTIONS' disabled for token`. Поэтому текст
            # называет все три: сказать «исчерпан лимит» на неверном ключе
            # значит отправить оператора ждать до завтра вместо того, чтобы
            # исправить ключ.
            #
            # Текст самой ДаData наружу не идёт и в лог не пишется НИКОГДА:
            # в него подставлен наш API-ключ, а учётные данные не попадают
            # ни в ответ, ни в журнал.
            return DirectoryQuotaExceeded(
                "Справочник адресов отклонил запрос: неверный ключ, "
                "неподтверждённая почта или исчерпанный дневной лимит"
            )
        if status == 429:
