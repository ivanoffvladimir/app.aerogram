...
@dataclass(frozen=True, slots=True)
class HealthResult:
    """Техническая доступность перевозчика под учётной записью тенанта.

    Требование системного ТЗ, раздел 9 (`health_check()` — «техническая
    доступность») и бэкенд-ТЗ, где метод описан как `account → health/latency`.
    В кабинете это кнопка «Проверить подключение» (фронт-ТЗ, раздел 8).

    **Сбой перевозчика здесь не исключение, а результат.** Метод отвечает
    на вопрос «работают ли доступы», и «не работают» — такой же законный
    ответ, как «работают». Исключение вместо ``is_healthy = False`` заставило
    бы вызывающего ловить его и превращать обратно в ответ, а по дороге
    ошибка перевозчика однажды дала бы 500 (CLAUDE.md §6).

    ``message`` — текст для человека, по-русски. У здоровой проверки его нет:
    подпись «всё хорошо» экран напишет сам.
    """

    is_healthy: bool
    #: Время вызова в миллисекундах. Меряется вокруг обращения к перевозчику,
    #: а не вокруг всей проверки: расшифровка учётных данных и запись итога —
    #: наше время, и приписывать его перевозчику неверно.
    latency_ms: int
...
class CarrierAdapter(Protocol):
    ...
    async def health_check(self, acc: CarrierAccount) -> HealthResult:
        """Проверить, что доступы тенанта работают (системное ТЗ, раздел 9).

        Проверяется **авторизованный вызов**, а не доступность хоста: пинг
        подтверждает, что интернет работает, и ничего не говорит о том,
        верны ли учётные данные, — а вопрос кабинета именно в этом.

        Вызов обязан быть дешёвым и ничего не создавать: запрос токена,
        чтение остатка квоты, список заказов. Расчёт для проверки не годится
        (он у части перевозчиков тарифицируется), создание — тем более.

        Ошибка перевозчика возвращается как ``is_healthy = False``, а не
        поднимается: см. ``HealthResult``.
        """
        ...
