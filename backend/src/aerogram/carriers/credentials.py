...
#: у Почты трекинг и оформление авторизуются порознь, — а домены всех
#: четырёх закрыты сетевой политикой окружения разработки, проверить нечем.
PENDING_CARRIERS: frozenset[str] = frozenset({"dellin", "pecom", "pochta", "yandex"})
