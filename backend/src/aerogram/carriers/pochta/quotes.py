...
суточную квоту. Поэтому набор продуктов
...
def build_tariff_payload(req: QuoteRequest, product: PochtaProduct) -> dict[str, Any]:
    ...
    if req.pickup:
        # У Почты в теле расчёта есть «courier — Отметка "Курьер"», но что
        # именно она включает — забор у отправителя, вручение получателю
        # или и то и другое, — не сказано ни на одной странице справки,
        # а метода подачи заявки на курьера в API «Отправка» нет вовсе.
        # Поставить отметку наугад значит посчитать не ту услугу, поэтому
        # цена считается для сдачи в отделение, и об этом говорится вслух:
        # молча выбросить просьбу забрать груз — обещание, которого мы
        # не давали, и цена, которой не будет в счёте.
        log.info("pochta.pickup_not_priced", product=product.code)
