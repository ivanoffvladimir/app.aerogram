"""Сторож окна аутентификации.

Политика ``users_auth_lookup`` (миграция 0002) открывает узкое окно на чтение таблицы
``users``, когда тенант ещё неизвестен. Компромисс приемлем ровно до тех пор, пока
окно открывается в одном месте — ``core.service.AuthService._lookup_user``.

Этот тест следит, чтобы второе место не появилось незаметно. Обоснование — ADR-0004.
"""
