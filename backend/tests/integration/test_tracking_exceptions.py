...
Часы подменяются намеренно, и **обе штуки**. Дедлайн эталонного запроса —
фиксированная дата, поэтому на настоящих часах тест «срок ещё не прошёл»
позеленел бы сегодня и покраснел бы в сентябре.

Вторые часы — приёма событий, и о них забыли, отчего два теста и сломались
восьмого сентября. ``TrackingService.ingest`` сравнивает время события
с ``utcnow`` СВОЕГО модуля и по порогу тишины (``STALE_AFTER``, пять суток)
помечает отправление зависшим. Событие в тестах датировано абсолютно,
и разрыв между ним и настоящим временем растёт каждый день: пока он был
меньше пяти суток, тесты проходили, а на шестые к причине разбора добавилась
третья, ``stalled``, которой они не ждали.

Отсюда ``frozen_ingest`` ниже — autouse, а не по вызову. Часы приёма нужны
замороженными КАЖДОМУ тесту файла, и фикстура, которую надо не забыть
попросить, ровно один раз и была забыта.
"""
...
#: Событие настолько старое, что порог тишины пройден с любой стороны —
#: и от замороженных часов приёма, и от настоящих.
LONG_AGO = datetime(2026, 7, 1, 9, 0, tzinfo=UTC)


#: Часы приёма событий. Момент один на весь файл и выбран после срока —
#: событиям тестов положено быть в прошлом относительно него, иначе
#: «доставлено» приезжает из будущего.
INGEST_NOW = AFTER_DEADLINE


@pytest.fixture(autouse=True)
def frozen_ingest(monkeypatch: pytest.MonkeyPatch) -> None:
    """Заморозить часы приёма событий.

    Без этого признак «зависло» считается от настоящего времени, а события
    в тестах датированы абсолютно: разрыв растёт с каждым днём, и однажды
    он переваливает за ``STALE_AFTER``. Тест при этом не расскажет, что
    сломался календарь, — он скажет «причин разбора три вместо двух».

    ``autouse`` намеренно: замораживать нужно всем, а фикстура, о которой
    надо помнить, уже один раз забылась.
    """
