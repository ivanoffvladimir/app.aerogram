...
BEYOND_CONTRACT: frozenset[str] = frozenset(
    {
        ...
        "/v1/auth/me",
        "/v1/auth/refresh",
        # Подключение второго фактора: контракт описывает только вход с кодом,
        # но выдавать секрет как-то надо (12.5 ТЗ, ADR-0018).
        "/v1/auth/mfa/setup",
        "/v1/auth/mfa/enable",
        "/v1/auth/mfa/disable",
        "/v1/users",
        ...
        # Разбор исключений: контракт описывает ленту одного отправления,
        # но раздел 10 ТЗ требует и вида «что горит» поперёк отправлений.
        "/v1/tracking/exceptions",
        # Сводка кабинета: экран /dashboard есть в ТЗ фронта, пути в контракте нет.
        "/v1/reports/summary",
        # Сверка расчёта и счетов: экран /invoices есть в ТЗ фронта (P2)
        # и Billing Lite — в продуктовом ТЗ, а пути в контракте нет.
        "/v1/billing/reconciliation",
        # Подписки на исходящие вебхуки: FR-3.6 требует, чтобы тенант
        # подписывался сам, но путей для этого в контракте нет — там описан
        # только ВХОДЯЩИЙ вебхук от перевозчика.
        ...
        # Чтение решения: контракт описывает только его создание. Прочитать
        # снимок нужно карточке отправления и разбору спора — иначе «каким
        # правилом выбрано» видно только в базе (ADR-0029).
        "/v1/decisions/{decision_id}",
        # Печатные формы. Фронт-ТЗ, раздел 7, требует «Documents: label/waybill
        # where available» в карточке отправления, а путей в замороженном
        # контракте нет (ADR-0016).
        "/v1/shipments/{shipment_id}/documents",
        "/v1/documents/{document_id}/content",
        # Правила маршрутизации: продуктовое ТЗ (раздел 7) их требует,
        # экран /routing-rules есть во фронт-ТЗ, а путей в замороженном
        # контракте нет. Состав языка правил — ADR-0028.
        "/v1/routing-rules",
        "/v1/routing-rules/{rule_id}",
    }
)

#: Пуст: все пути контракта реализованы. Последним закрылся приём вебхуков
#: перевозчика — узким окном поиска по ADR-0015.
